...
public class SapphireElytraRenderer<E extends LivingEntity, M extends EntityModel<E>> extends ElytraLayer<E, M> {
    private static final ResourceLocation TEXTURE = new ResourceLocation(CrystalMod.MOD_ID, "textures/entity/sapphire_elytra.png");
    private final ElytraModel<E> model;

    public SapphireElytraRenderer(RenderLayerParent<E, M> pRenderer, EntityModelSet pModelSet) {
    ...
    public void render(PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight, E pLivingEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTicks, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        ...
        if(itemStack.is(ModItems.SAPPHIRE_ELYTRA.get())) {
